    ...
    def _update_pv_history():
        ...
        try:
            ...
            for e in PvExcessControl.instances.values():
                inst = e["instance"]
                if _get_state(inst.appliance_switch) == "on":
                    current_appliance_pwr_load = current_appliance_pwr_load + _get_num_state(inst.actual_power)
            log.debug(f"Update_pv_history actual total appliance power: {current_appliance_pwr_load}W")
            ...
            if PvExcessControl.import_export_power:
                ...
                load_pwr = pv_pwr + excess_pwr - int(current_appliance_pwr_load)
            else:
